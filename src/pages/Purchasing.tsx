...
export default function Purchasing() {
  ...
  useEffect(() => {
    ...
    const unsubscribePO = onSnapshot(collection(db, 'purchase_orders'), (snapshot) => {
      const poData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      // Sort by date descending
      poData.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const handleOpenModal = (order: any = null) => {
    if (order) {
      ...
      const newPoNumber = `PO-${new Date().getTime().toString().slice(-6)}`;
      setFormData({ ...initialFormState, poNumber: newPoNumber });
    }
    setIsModalOpen(true);
  };
  ...
  const handleItemChange = (index: number, field: string, value: any) => {
    ...
    if (field === 'itemId') {
      ...
      if (selectedInvItem) {
        ...
        item.price = selectedInvItem.cost; // Use cost for PO
      }
    } else {
      item[field] = value;
    }
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {purchaseOrders.map((order) => (
              <tr key={order.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600">{order.poNumber}</td>
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-4xl w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">{editingOrder ? 'Edit Purchase Order' : 'Create Purchase Order'}</h2>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase w-32">Unit Cost</th>
